  ...
  stages {
      stage('Execute Commands') {
        steps {
          ...
          script {
            // Execute commands inside the Docker container
            def returnStatus = sh returnStdout: true, script: 'docker exec -e EC2_INSTANCE_IP=127.0.0.1 b622 /bin/bash -c "source /opt/myvenv/bin/activate && cd /opt/myproject/tests && pytest && echo \$?"'
